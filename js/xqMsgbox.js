...
\t</div>
    ...
    this.open = async function (btns = 0, title = "Message", content = "") {
        ...
        if (btns.constructor.name === "Array") {
        ...
        for (let closer of closers) closer.addEventListener("click", (e) => { this.close() });
        return await new Promise(resolve => {
            resolver = resolve
        })
    }
    this.close = function () {
        elem.addEventListener("animationend", (e) => { document.body.removeChild(elem) })
    ...
    function createButton(id, text) {
        ...
        btn.addEventListener("click",(e)=>{  resolver(id) })
        return btn
    }
}